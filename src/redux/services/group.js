...
export const groupService = createApi({
  ...
  endpoints: (builder) => ({
    ...
    createGroup: builder.mutation({
      queryFn: (args) => groupApi.createGroup(args.name),
      invalidatesTags: () => [{ type: "Groups", id: "LIST" }],
    }),
    addGroupAttribute: builder.mutation({
      queryFn: (args) => groupApi.addAttribute(args.group, args.attr),
      invalidatesTags: (result, error, { id }) => [{ type: "Groups", id }],
    }),
    deleteGroupAttribute: builder.mutation({
      queryFn: (args) => groupApi.removeAttribute(args.group, args.attr),
      invalidatesTags: (result, error, { id }) => [{ type: "Groups", id }],
    }),
  }),
});
